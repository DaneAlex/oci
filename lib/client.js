...
client.prototype.getCurrentUsers = function getCurrentUsers(callback){
    var opts = {
        url: this.OwncastUrl + "/api/admin/clients",
        streamKey: this.StreamKey,
        method: "GET"
    }
    ...
    return api.api(opts, (err, res, jsonBody) => {
        ...
        log("INFO", "Get Current Clients Succeeded");
        return callback(jsonBody);
    });

}

client.prototype.hideMessage = function hideMessage(id, callback){
    var opts = {
        url: this.OwncastUrl + "/api/admin/chat/updatemessageavailability",
        streamKey: this.StreamKey,
        method: "POST",
        body: {"visible": false, "idArray": id}
    }
